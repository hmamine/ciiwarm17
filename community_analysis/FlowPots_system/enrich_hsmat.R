...
#*********sensistive-depleted
hsdep=c("afull","hs")
physeq.hs=subset_samples(physeq, config2 %in% hsdep)
comm=c("Back","Comp")
physeq.hs=subset_taxa(physeq.hs, tag %in% comm)

#******matrix-compartment
dep=c("HSDepletedMatrix")
condmat=c("matrix")
...
c.mat1 = makeContrasts ( config3matrix.full	-	config3matrix.hs, levels = finalMod1)
...
DT_1p=fit1$p.value
	...
	setnames(DT_1, "config3matrix.full - config3matrix.hs", "fc.FvsS")
	setnames(DT_1p, "config3matrix.full - config3matrix.hs", "p.FvsS")
...
com1$ph.p.fc.test <- ifelse(com1$ph.p.fc.test ==0,"Non_Significant", ifelse( com1$ph.p.fc.test==1, "Actinobacteria", ifelse(com1$ph.p.fc.test ==2, "Bacteroidetes", ifelse(com1$ph.p.fc.test== 3, "Firmicutes", "Proteobacteria"))))
	...
	ggtitle(paste0("Sensitive-depleted-",condmat))
...
p2
#dev.off()
#pdf(paste0("Sensitive-depleted-",condmat,".pdf"), useDingbats=FALSE,width = 13, height = 13)
print(p3)
#dev.off()
