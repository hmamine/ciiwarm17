...
#***shaking
dep=c("HCDepletedShaking")
submat=c("unstrtsb")

physeq.hcsk=subset_samples(physeq.hc, config5 %in% submat)

#***differentially abundant CTU 
m = as(otu_table(physeq.hcsk), "matrix") + 1L
t = data.frame(as(tax_table(physeq.hcsk), "matrix"))
T=AnnotatedDataFrame(t)
s = as(sample_data(physeq.hcsk), "data.frame")
