...
#***standing
dep=c("HCDepletedStanding")
submat=c("strtsb")

physeq.hcst=subset_samples(physeq.hc, config5 %in% submat)

#***differentially abundant CTU 
m = as(otu_table(physeq.hcst), "matrix") + 1L
t = data.frame(as(tax_table(physeq.hcst), "matrix"))
T=AnnotatedDataFrame(t)
s = as(sample_data(physeq.hcst), "data.frame")
...
c.mat1 = makeContrasts ( config2afull	-	config2hc, levels = finalMod1)
...
DT_1p=fit1$p.value
	...
	setnames(DT_1, "config2afull - config2hc", "fc.FvsC")
	setnames(DT_1p, "config2afull - config2hc", "p.FvsC")
...
com1$ph.p.fc.test <- ifelse(com1$ph.p.fc.test ==0,"Non_Significant", ifelse( com1$ph.p.fc.test==1, "Actinobacteria", ifelse(com1$ph.p.fc.test ==2, "Bacteroidetes", ifelse(com1$ph.p.fc.test== 3, "Firmicutes", "Proteobacteria"))))
	...
	ggtitle(paste0("Competitive-depleted-",submat))
...
p2
#dev.off()
#pdf(paste0("Competitive-depleted-",submat,".pdf"), useDingbats=FALSE,width = 13, height = 13)
print(p3)
#dev.off()
