...
#***shaking
dep=c("HSDepletedShaking")
submat=c("unstrtsb")
physeq.hssk=subset_samples(physeq.hs, config5 %in% submat)


#***differentially abundant CTU 
m = as(otu_table(physeq.hssk), "matrix") + 1L
t = data.frame(as(tax_table(physeq.hssk), "matrix"))
T=AnnotatedDataFrame(t)
s = as(sample_data(physeq.hssk), "data.frame")
